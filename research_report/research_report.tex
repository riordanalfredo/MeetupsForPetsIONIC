...
The case study, Meetups for Pets, was a cross-platform Ionic 3 app as specified
earlier. It incorporated quite a few mobile device features, such as accessing the
native SMS app and the camera. Initially, these features were thought to be difficult to
migrate, but it turns out that the code dealing with these pieces of functionality could be
ported over directly for the most part. The main pieces of code the tool needed to deal with
turned out to be specific to the Ionic framework itself, as opposed to Cordova plugins/bridges for
interacting with native device functionality.
\newline \newline
The migration tool model involved a central Migrator entity that interacted with different components
of the Ionic 3 framework that each needed specific changes to be made to them to be converted into Ionic 4
components. These components were modeled as separate entities themselves, allowing for future updates to be done
by simply creating another entity for the required component.
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.8\linewidth,height=5cm,keepaspectratio]{model.png}
    \caption{Ionic Migration Tool Model}
    \label{fig:ionicModel}
\end{figure}
\newline \newline
...
\begin{figure}[!htb]
    \centering
    \subfloat[Ionic 3]{
        \includegraphics[width=0.8\linewidth,height=0.6cm]{button.PNG}
        \label{fig:ion3button}
    }
    \qquad
    \subfloat[Ionic 4]{
        \includegraphics[width=0.8\linewidth,height=1.5cm]{buttonv4.PNG}
...
\newline \newline
According to Martinez et al.\cite{b2} study is related to our work in terms of processing different kinds of 
...
\newline \newline
In the paper \cite{b5}, author describes the process of migrating a large-scale software application from Mainframe to J2EE
...
\newline \newline
Lastly, Aboulsamh describes \cite{b6} the feasibility of model-driven approaches to be used in the software migration context in general.
...
\bibitem{b2} Martinez, L., Pereira, C., Favre, L., ``Migrating C/C++ Software to Mobile Platforms in the ADM context,'' \textit{International Journal of Interactive Multimedia and Artificial Intelligence}, Vol. 4 , N\textsuperscript{o}3, p. 34, 2017. [Abstract].
...
\bibitem{b5} Fleurey, F., Breton, E., Baudry, B., Nicolas, A., Jezequel, J., ``Model-Driven Engineering for Software Migration in a Large Industrial Context'', in International Conference on Model Driven Engineering Languages and Systems, 2007, pp. 482-497, MODELS
...
\bibitem{b7} R. Gronback, "Eclipse Modeling Project | The Eclipse Foundation", Eclipse.org, 2019. [Online]. Available: \url{https://www.eclipse.org/modeling/emf/}. [Accessed: 04- Nov- 2019].
\bibitem{b8} "Sirius - Overview", Eclipse.org, 2019. [Online]. Available: \url{https://www.eclipse.org/sirius/overview.html}. [Accessed: 04- Nov- 2019].
\bibitem{b9} "Acceleo | Home", Eclipse.org, 2019. [Online]. Available: \url{https://www.eclipse.org/acceleo/}. [Accessed: 04- Nov- 2019].
